...
class V2Vchannels:              
    ...
    def update_shadow(self, delta_distance_list):
        ...
        if len(delta_distance_list) == 0: #初始化语句在这里，和V2I的定义和调用方式一样
...
class Environ:
    ...
    def __init__ (self, down_lane, up_lane, left_lane, right_lane, width, height):
        self.timestep = 0.01 #时隙是0.01s 10ms作为一个时隙
        ...
        self.width = width
        self.height = height
        self.vehicles = []  #用于存放class vehicles
        self.demands = []  
        ...
        self.sig2_dB = -114 #噪声的DB形式
        self.bsAntGain = 8  #基站的天线增益
        self.bsNoiseFigure = 5 #基站噪声系数
        self.vehAntGain = 3 #车辆天线增益
        self.vehNoiseFigure = 9 #车辆天线增益
        self.sig2 = 10**(self.sig2_dB/10)  #噪声的功率
        self.V2V_Shadowing = []
        self.V2I_Shadowing = []
        self.delta_distance = [] #用于存储车辆之间的相互距离
        self.n_RB = 20
        self.n_Veh = 40 #使用20个信道 40个车辆？ 在调用函数new_random_game的时候才真正调用，如果没有给envi.new_random_game()赋值，那么默认是使用self.n_Veh = 40 (这里的步骤有些繁杂，可以直接省去)
        self.V2Vchannels = V2Vchannels(self.n_Veh, self.n_RB)  # number of vehicles
        self.V2Ichannels = V2Ichannels(self.n_Veh, self.n_RB)
        #需要注意的是，这里定义了三个class，分别是vehicles\V2Vchannels\V2Ichannels，一个针对信道，一个针对车辆本身
    ...
    def add_new_vehicles_by_number(self, n):  #在内部的大循环中调用add_new_vehicles函数，往self.vehicles中添加Vehicle类
        for i in range(n):
        ...
        self.V2V_Shadowing = np.random.normal(0, 3, [len(self.vehicles), len(self.vehicles)]) #生成一个均值为0，方差为3的正态分布20*20矩阵----考虑到了车辆和车辆之间的阴影衰落
        self.V2I_Shadowing = np.random.normal(0, 8, len(self.vehicles)) # 生成一个均值为0，方差为8的正太分布1*20矩阵
        #但是这个V2V_Shadowing矩阵中存储的到底是什么？
    ...
    def update_large_fading(self, positions, time_step):
        #是否可以这样认为，大尺度衰落考虑路径损耗和阴影衰落，小尺度衰落一般只考虑快衰落
    ...
    def renew_channel(self): #通过调用renew_channel函数，把V2V/V2Ichannels类的衰落激活了
        # ===========================================================================
    ...
    def renew_channels_fastfading(self): #在考虑快衰落的时候，renew信道
        # =======================================================================
        # This function updates all the channels including V2V and V2I channels
        # =========================================================================
        self.renew_channel()
        self.V2Ichannels.update_fast_fading()
        self.V2Vchannels.update_fast_fading()
        #是否考虑快衰落，和只考虑慢衰落是可以独立使用的，在考虑快衰落的时候，self.V2Vchannels变成了self.V2Vchannels_with_fastfading,由于是类内元素，在后面的程序中就调用这个函数了
    ...
    def Compute_Performance_Reward_fast_fading_with_power_asyn(self, actions_power):   # revising based on the fast fading part  ？这里asyn是异步的意思吗？
    ...
    def act_for_training(self, actions, idx):  # 主架构中获取奖励是从这里获取的
        ...
        #return t
        return t - (self.V2V_limit - time_left)/self.V2V_limit  #如文献中所说，lamda_p = 1
        
    def act_asyn(self, actions):  #异步动作？
        self.n_step += 1
        if self.n_step % 10 == 0:
            self.renew_positions()            
            self.renew_channels_fastfading()
    ...
    def new_random_game(self, n_Veh = 0):  #相当于很多的东西在这里重新被定义了一遍
        # make a new game
        self.n_step = 0
        self.vehicles = []
        if n_Veh > 0:
        ...
        self.activate_links = np.zeros((self.n_Veh,3), dtype='bool')  #猜测使用bool形变量来表示60条链路是否通信成功

        #运行到这里，便有了self.V2V/V2Ichannels_wtih_fastfading、self.veh这三个主要的类
