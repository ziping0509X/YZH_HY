...
class Agent(BaseModel):
    ...
    # =============
        ...
        V2V_channel = (self.env.V2V_channels_with_fastfading[idx[0],self.env.vehicles[idx[0]].destinations[idx[1]],:] - 80)/60  #这是求了一次平均吗？
        V2I_channel = (self.env.V2I_channels_with_fastfading[idx[0], :] - 80)/60
        V2V_interference = (-self.env.V2V_Interference_all[idx[0],idx[1],:] - 60)/60  # 初次写代码，只保留这三个作为状态就行
    ...
    # --- Building the DQN -------
        ...
        with tf.variable_scope('prediction'):
            ...
            self.q_action = tf.argmax(self.q, dimension = 1) #self.q_action是所有Q值最大的哪一个对应的标号，那为什么要取[0]?
